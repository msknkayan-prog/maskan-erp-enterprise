/* Maskan ERP Enterprise — maintenance patch 2026-08-07
   Fixes ambiguous Supabase relationships without altering the main application bundle.
*/

// Projects: force the intended project_manager_id -> profiles relationship.
...
// Approval history: force acted_by -> profiles relationship.
activity = async function(){
  ...
  ${rowsTable(['التاريخ','المستخدم','نوع المعاملة','الإجراء','من حالة','إلى حالة','الملاحظات'],(data||[]).map(x=>`<tr>
    <td>${d(x.created_at)}</td><td>${esc(x.actor?.full_name||'—')}</td><td>${esc(x.entity_type)}</td>
